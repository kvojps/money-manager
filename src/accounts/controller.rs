...
pub fn handle_add(conn: &Connection, name: &str, value: &str) -> Result<String, String> {
    ...
    add_account(conn, name, cents)
        .map_err(|err| format!("Erro ao salvar a conta: {err}"))?;

    Ok(render_add_account(name, cents))
}

pub fn handle_list(conn: &Connection) -> Result<String, String> {
    let rows = list_accounts(conn).map_err(|err| format!("Erro ao listar contas: {err}"))?;
    Ok(render_list_accounts(&rows))
}
